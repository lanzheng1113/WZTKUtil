...
#include "util/MD5ChecksumDefines.h"
#include <strsafe.h>
#include <fstream>
#include <cassert>
...
*****************************************************************************************/
wstring CMD5Checksum::GetMD5(const wstring& strFilePath)
{
	/*
	const int BUFFER_SIZE = 4096;
	std::string strReturn = "";
	std::ifstream fileToRead;
	fileToRead.open(fullpath.c_str(), ios::binary);
	if (fileToRead.is_open())
	{
	char tmp[BUFFER_SIZE];
	while (!fileToRead.eof())
	{
	fileToRead.read(tmp, BUFFER_SIZE);
	strReturn.append(tmp, fileToRead.gcount());
	}
	fileToRead.close();
	}
	return strReturn;
	*/
	...
	{ 
		...
		if(!file.is_open())
		{
			//AM_ERROR("DuoUpdate","打开文件%S失败了，LastError=%d",strFilePath.c_str(),GetLastError());
			return strRet;
		}
		...
		strRet = String(MD5Checksum.Final()).toStdWString();
	}
	//report any file exceptions in debug mode only
	catch (...)
	{ 
		//AM_ERROR("DuoUpdate","计算文件的MD5失败了，LastError=%d",GetLastError());
	}
	return strRet;
}
...
{ 
	...
	{ 
		...
		{ 
			char szTemp[32] = {0};
			StringCbPrintfA(szTemp,sizeof(szTemp),"0%X",lpszMD5[i]);
			Str = szTemp;
		}
		else
		{ 
			char szTemp[32] = {0};
			StringCbPrintfA(szTemp,sizeof(szTemp),"%X",lpszMD5[i]);
			Str = szTemp;
		}
...
string CMD5Checksum::GetMD5OfString( const string& strString )
{
	std::string strRet = "";
	try
	{ 
		...
		MD5Checksum.Update( Buffer, nLength );
		strRet = MD5Checksum.Final();
	}
	//report any file exceptions in debug mode only
	catch ( ... )
	{
		strRet = "";
		printf("CMD5Checksum::GetMD5: CFileException caught"); 
	}
	return strRet;
}

string CMD5Checksum::GetMD5( char* pBuff, unsigned long nleng )
{
	//参数效验
	if (pBuff == NULL || nleng <= 0)
		return NULL;
	std::string strRet = "";
	try
	{ 
